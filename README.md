...
Coding conventions are documented in [`.github/copilot-instructions.md`](.github/copilot-instructions.md) and the focused files in [`.github/instructions`](.github/instructions). In particular, comments should explain intent rather than restate code; exported data-layer functions require TSDoc/JSDoc for their purpose, parameters, and return values; and reusable Astro components document their `Props` contract. TypeScript formatting is enforced by ESLint with two-space indentation, single quotes, semicolons, multiline trailing commas, and spaces inside braces.
...
The shared **Issue Triage Board** canvas (`.github/extensions/issue-triage-board/`) ranks open GitHub issues, highlights the three most likely to need attention, and lets developers attach an issue's details to the current Copilot session. It uses the authenticated `gh` CLI to read issues from the current repository.

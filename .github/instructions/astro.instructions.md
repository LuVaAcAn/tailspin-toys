...
- Document every reusable component's `Props` interface with a brief TSDoc comment describing the component contract and any non-obvious prop constraints.
...
- Add comments only for intent, accessibility reasoning, or non-obvious implementation decisions. Do not restate markup or expressions.
- Keep component contract documentation current when changing `Props` or component behavior.
